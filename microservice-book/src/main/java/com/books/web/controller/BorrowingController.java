...
public class BorrowingController {
	...
	public List<Borrowing> listBorrowings() {
		...
		if (borrowings.isEmpty())
			throw new BorrowingNotFoundException("Les emprunts de livres n'ont pas été retrouvés.");
	...
	public ResponseEntity<Borrowing> addBorrowing(@Valid @RequestBody Borrowing borrowing) {
		...
		List<Booking> bookingBook = bookingDao.findBookingByBookAndStateOrderByDateCreate(bookBorrowed,
				State.EnAttente);
		...
		if (bookingBook.size() > 0) {
			...
			if (borrowingIdUser.equals(bookingIdUser)) {
				...
				if (newBorrowing == null)
					throw new CanNotAddBorrowingException("Impossible d'ajouter l'emprunt.");
	...
	public ResponseEntity<Borrowing> renewalBorrowing(@PathVariable int id) {
		...
		if (!borrowing.isPresent())
			throw new BorrowingNotFoundException("L'emprunt avec l'id : " + id + " n'a pas été retrouvé.");
		...
		if (today.after(borrowingRenewal.getDeadline())) {
			throw new BorrowingInvalidRenewalException("Borrowing02");
		}
	...
	public ResponseEntity<Borrowing> returnBorrowing(@PathVariable int id) {
		...
		if (bookings.size() > 0) {
			...
			String mailText = "Bonjour " + user.getFirstName() + " " + user.getLastName() + ","
					...
					+ bookReturn.getName() + "\n\nVous avez jusqu'au " + dateFormat.format(deadlineBooking)
					+ " pour venir récupérer votre document."
	...
	public Response deleteBorrowing(@PathVariable int id) {
		...
		if (!borrowing.isPresent())
			throw new BorrowingNotFoundException("Aucun prêt n'a été retrouvé.");
